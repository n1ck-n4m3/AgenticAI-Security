...
def train(model: MyGAT, train_loader, criterion, optimizer, device):
    ...
    for data in train_loader:
        x, y, edge_index, edge_attr = data.x.to(device).float(), data.y.to(device), data.edge_index.to(device), data.edge_attr.to(device).float()
        x = edge_attr[:, 0, :]
...
def parse_arguments():
    ...
    if args.dataset == "mmlu": 
        args.dataset_path = "./ModelTrainingSet/mmlu/dataset1.pkl"
    elif args.dataset == "csqa": 
        args.dataset_path = "./ModelTrainingSet/csqa/dataset1.pkl"
    elif args.dataset == "gsm8k": 
        args.dataset_path = "./ModelTrainingSet/gsm8k/dataset1.pkl"
    else: 
    ...
    filename = f"{current_time_str}-defend_type_Gsafe_hiddim_{args.hidden_dim}-heads_{args.num_heads}-layers_{args.num_layers}-epochs_{args.epochs}-lr_{args.lr}-dropout_{args.dropout}-wd_{args.weight_decay}.pth"
