    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            if defend_type in ["SCL", "TAM"]:
                ...
                score = model.get_anomaly_scores(x, edge_index)
                encoded = model.encode(x, edge_index)
                loss = F.mse_loss(encoded, x)
...
def parse_arguments():
    ...
    filename = f"{current_time_str}-defend_type_{args.defend_type}-hiddim_{args.hidden_dim}-latent_{args.latent_dim}-heads_{args.num_heads}-layers_{args.num_layers}-epochs_{args.epochs}-lr_{args.lr}-temp_{args.temperature}.pth"
    
    if args.defend_type == "PREM":
        filename = f"{current_time_str}-defend_type_{args.defend_type}-hiddim_{args.hidden_dim}-k_{args.prem_k}-alpha_{args.prem_alpha}-gamma_{args.prem_gamma}-epochs_{args.epochs}-lr_{args.lr}.pth"
...
def main():
    ...
    print(f"test batch: {test_batch_size}")
